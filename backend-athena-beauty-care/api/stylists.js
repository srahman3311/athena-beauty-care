...
router.post("/update", async (request, response) => {
    ...
    Stylist.findOne({ email }, (err, Stylist) => {
        ...
        Stylist.calendarAccessCode = refreshToken;
...
router.post("/login", (request, response) => {
    ...
    Stylist.findOne({ email }, (dbError, Stylist) => {
        ...
        if(!Stylist) return response.status(404).json({msg: "Stylist Doesn't Exist"});
            ...
            if(isMatch) {
                ...
                jwt.sign(
                    ...
                    (jwtError, token) => {
                        ...
                        response.status(200).json(Stylist);
                    }
                );
            } 
...
router.post("/fetch-events-by-mail", async (request, response) => {
    ...
    Stylist.findOne({email}, async (error, Stylist) => {
        ...
        try {
            ...
            // Get a new token each time before sending get request for calendar resources (events or whatever)
            const ep1 = `https://oauth2.googleapis.com/token?client_id=${process.env.CLIENT_ID}&client_secret=`
            const ep2 = `${process.env.CLIENT_SECRET}&refresh_token=${Stylist.calendarAccessCode}&grant_type=refresh_token`; 
            const tokenEndpoint = ep1 + ep2;
...
router.post("/add-user", (request, response) => {
    ...
    // Else, new add request has been made
        ...
        connection.query("select * from users where username=?", [username], (err, user) => {
            ...
            if(designation === "") designation = "Stylist";
